...
# Ample time for Sequencer to start
sleep 5

# check the logs to see if the Sequencer is running
docker compose -f docker-compose-buildtest.yml logs sequencer
...
curl -X POST -H "content-type:application/json" http://localhost:15672/api/queues/%2f/ProduceTest/get -d '{"vhost":"/","name":"ProduceTest","truncate":"50000","ackmode":"ack_requeue_false","encoding":"auto","count":"1"}' -u guest:guest| jq -r '.[].payload' | grep -q '[{"test":"rabbitmq"}]'
