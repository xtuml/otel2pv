...
var (
	...
	_ io.WriteCloser = &AtomicFile{}
)

// AtomicFile allows writing to a file atomically
...
func (f *AtomicFile) handleError(err error) error {
	if err == nil {
		return nil
	}
...
func (f *AtomicFile) SetWriteDeadline(t time.Time) error {
	if f.err != nil {
		return f.err
	}
...
func (f *AtomicFile) Truncate(size int64) error {
	if f.err != nil {
		return f.err
	}
...
func (f *AtomicFile) Seek(offset int64, whence int) (ret int64, err error) {
	if f.err != nil {
		return 0, f.err
	}
...
func (f *AtomicFile) WriteAt(b []byte, off int64) (n int, err error) {
	if f.err != nil {
		return 0, f.err
	}
...
func (f *AtomicFile) WriteString(s string) (n int, err error) {
	if f.err != nil {
		return 0, f.err
	}
...
func (f *AtomicFile) alreadyClosed() bool {
...
func (f *AtomicFile) RemoveIfNotClosed() {
	if f == nil {
		return
	}
	if f.alreadyClosed() {
